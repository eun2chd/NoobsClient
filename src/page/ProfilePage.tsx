...
const ProfilePage: React.FC = () => {
  ...
  const updateUserStats = (id: number) => {
    setIsLoading(true);
    axios
      .post("http://www.noobsapp.store:8000/noobs/friendUserBrUpdate", { user_id: id })
      .then(() => {
        Swal.fire({
          icon: "success",
        ...
        return axios.post("http://www.noobsapp.store:8000/noobs/UserDetilsInfo", {
          gameid: id,
        });
      })
      .then((response) => {
      ...
      const userDetailsRequest = async () => {
        try {
          const response = await axios.post(
            "http://www.noobsapp.store:8000/noobs/UserDetilsInfo",
            {
              gameid: user_id,
            }
          );
